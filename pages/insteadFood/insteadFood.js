...
Page({
  ...
  // 跳转到子商品购买页面
  goToTheInsteadBookPage() {
    utils.userIsLogin().then(_ => {
      let goods = this.data.goods;
      let mid = goods.goodsBase.mid;
      let shopName = goods.goodsInfo.name;
      let gid = goods.goodsInfo.gid;
      let giid = this.data.giid;
      utils.navigateTo('/pages/insteadBook/insteadBook?mid=' + mid + '&shopName=' + shopName + '&gid=' + gid + '&giid=' + giid);
    }).catch(_ => {
      console.log('unLogin');
    })
  }
